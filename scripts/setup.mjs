...
(async () => {
  ...
  if (fs.existsSync(configDir) && fs.existsSync(templatePackageJsonPath)) {
    console.log(`Applying configuration from ${response.config} to the root...`);
    ...
    console.error(`Error: Configuration directory "${configDir}" or its package.json not found.`);
  }
})();
